...
The supported Operating Systems Windows Server 2016 and Windows 10.
...
try {
    ...
    if (!$CheckRegistery) {
       ...
       $obj"
            ...
            #Setting up Tenant Group RDS Context
...
installAgent -RDBrokerURL $RDBrokerURL -DefinedTenantGroupName $DefinedTenantGroupName -TenantName $TenantName -HostPoolName $HostPoolName -Description $Description -FriendlyName $FriendlyName `
-Hours $Hours -rdshIs1809OrLater $rdshIs1809OrLater -ActivationKey $ActivationKey `
-TenantAdminUPN $TenantAdminUPN -TenantAdminPassword $TenantAdminPassword -localAdminUserName $localAdminUserName `
-registrationToken $registrationToken -localAdminPassword $localAdminPassword


' -RdBrokerURL \"', parameters('rdBrokerURL'), '\"', ' ', ' -DefinedTenantGroupName \"', 
parameters('existingTenantGroupName'), '\"',' ',' -TenantName \"', parameters('existingTenantName'), '\"',' ', ' -HostPoolName \"', 
parameters('hostPoolName'), '\"',' ', ' -FriendlyName \"', variables('HostPoolFriendlyName'), '\"',' ', ' -Description \"', variables('HostPoolDescription'), '\"',' ', 
' -Hours \"', variables('registrationExpirationHours'), '\"',' ', ' -TenantAdminUPN \"', parameters('tenantAdminUpnOrApplicationId'), '\"',' ', 
' -TenantAdminPassword \"', parameters('tenantAdminPassword'), '\"',' ',' -localAdminUsername \"', variables('existingDomainUsername'), '\"',' ', 
' -localAdminpassword \"', parameters('existingDomainPassword'), '\"',' ', ' -rdshIs1809OrLater \"', parameters('rdshIs1809OrLater'), '\"', ' ', 
' -isServicePrincipal \"', parameters('isServicePrincipal'), '\"', ' ', ' -aadTenantId \"', parameters('aadTenantId'), '\"', ' ', 
