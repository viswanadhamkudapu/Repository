...
if ($LoadBalancingBool) {
    ...
    if ($DeploymentStatus.ProvisioningState -eq "Succeeded") {
        ...
        if (!$DFVariable) {
            $Webhook = New-AzureRmAutomationWebhook -Name $DepthFirstWebhook -RunbookName $runbookName -IsEnabled $True -ExpiryTime (get-date).AddYears(5) -ResourceGroupName $ResourceGroupName -Force
            $WebhookURI = $Webhook.WebhookURI | Out-String
